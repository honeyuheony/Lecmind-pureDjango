...
def set_concentrate(img):
    ...
    gaze_ratio_left_eye = get_gaze_ratio([36, 37, 38, 39, 40, 41], landmarks, gray, img)
    gaze_ratio_right_eye = get_gaze_ratio([42, 43, 44, 45, 46, 47], landmarks, gray, img)
    gaze_ratio = (gaze_ratio_left_eye + gaze_ratio_right_eye) / 2
    eye_direction_sum += gaze_ratio
    print(gaze_ratio)

    # 고개 돌리는 방향 감지
    head_direction = get_head_angle_ratio([27, 28, 29, 30, 31, 32, 33, 34, 35], landmarks, img)
    ...
    for face in faces:
        ...
        if temp_faces_for_compare[2]:
            temp_faces_for_compare = compare_faces(img, num_faces, temp_faces_for_compare)
